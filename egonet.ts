...
class EgoGraph {
    ...
    async fetchAutocomplete(term: string, maxCount: number) : Promise<Set<string>> {
        ...
        if (res.status === Status.OK) {
            ...
            for (let hit of hits[1].slice(0, maxCount)) hit.split(this.pattern).slice(1).map((t: string) => {
                if (!new RegExp('^[0-9.]+$').test(t)) set.add(t); // filters
            });
            return set;
        } else {
    ...
    async build() {
        ...
        for (let depth = 0; depth < this.depth; depth++) {
            ...
            for (let i=0; i < sources.length; i++) {
                ...
                if (!this.graph.getNode(src)) this.graph.addNode(src, {count: 1, depth: src === this.query ? 0 : depth + 1}); // new node
                ...
                targets.forEach((target: string) => {
                    ...
                    if (!targetNode) {
                        ...
                        this.graph.addLink(src, target, {distance: dist, weight, query: `${src}${this.pattern}${target}`}); // new edge
                        ...
                        if (link1 || link2) link1 ? link1.data.weight+=weight : link2.data.weight+=weight;
                        else this.graph.addLink(src, target, {distance: dist, weight, query: `${src}${this.pattern}${target}`}); // existing edge
                    }
                    weight -= 1;
                    distance += 1;
                });
...
const handleOptions = async (req: ServerRequest): Promise<void> => {
    console.log(`${req.method} ${req.url}`);
    const headers = new Headers(); // CORS
    headers.set('Access-Control-Allow-Origin', 'https://ego.jveres.me');
...
const handleQuery = async (req: ServerRequest, options: EgoGraphOptions): Promise<void> => {
    console.log(`${req.method} ${req.url}`);
    let cache = httpCache.get(options.query);
    if (!(cache && (Date.now() - cache.date) < CACHE_EXPIRATION_MS)) { // not found in cache or expired
        const ego = new EgoGraph({query: options.query, depth: options.depth || EgoGraph.DEFAULT_GRAPH_DEPTH, radius: options.radius || EgoGraph.DEFAULT_GRAPH_RADIUS});
        await ego.build();
        console.info(`${req.method} ${req.url} ${cache ? 'Refreshed in cache' : 'Cached'} at ${httpCache.size}`);
        cache = {date: Date.now(), value: JSON.stringify(ego.toJSON())};
        httpCache.set(options.query, cache);
        if (httpCache.size > MAX_CACHE_CAPACITY) httpCache.delete(httpCache.keys().next().value); // rotate cache
...
const handleError = async (req: ServerRequest, message: string): Promise<void> => {
...
(async () => {
    for await (const req of server) {
        ...
        if (req.method === 'OPTIONS') {
            ...
            handleQuery(req, {
                query: params.get('q') || '',
                depth: Number(params.get('d')) || EgoGraph.DEFAULT_GRAPH_DEPTH,
                radius: Number(params.get('r')) || EgoGraph.DEFAULT_GRAPH_RADIUS
            })
            .catch(async ({message}) => {
                try {
